...
const CAPTURE_FLAG: String = "--capture-hud-character-slices"
const SCENE_PATH: String = "res://scenes/dev/hud_character_vertical_slice_gallery.tscn"
const OUTPUT_DIRECTORY: String = "res://dev_doc/ui-art-research/hud-ghv2-character-slice/evidence"
...
func _initialize() -> void:
	print("=== capture_hud_character_vertical_slice_gallery ===")
	if DisplayServer.get_name() == "headless":
		print("HUD_CHARACTER_CAPTURE_UNSUPPORTED_HEADLESS")
		quit(1)
		return
	if not CAPTURE_FLAG in OS.get_cmdline_user_args():
		print("HUD_CHARACTER_CAPTURE_FLAG_REQUIRED")
		quit(1)
...
func _capture_one(packed: PackedScene, capture_size: Vector2i,
	...
	viewport.name = "CharacterCaptureViewport_%dx%d" % [capture_size.x, capture_size.y]
	...
	if not _all_states_inside_safe_area(gallery):
		_fail("%dx%d 有角色栏状态板超出安全画布" % [capture_size.x, capture_size.y])
		viewport.queue_free()
		await process_frame
		return
	...
	var filename := "hud_character_slice_%dx%d.png" % [capture_size.x, capture_size.y]
...
func _all_states_inside_safe_area(gallery: Control) -> bool:
	...
	for state_name: String in STATE_NAMES:
		var panel: Control = gallery.get_node_or_null(
			"SafeArea/Layout/Content/CharacterColumn/%s/CharacterHudPanel" % state_name) as Control
		if panel == null or not _contains_rect(safe_rect, panel.get_global_rect()):
			return false
	return true
...
func _finish() -> void:
	print("HUD_CHARACTER_CAPTURE_OK" if not _failed else "HUD_CHARACTER_CAPTURE_FAILED")
