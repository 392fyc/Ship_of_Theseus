...
const CAPTURE_FLAG: String = "--capture-hud-vertical-slices"
const SCENE_PATH: String = "res://scenes/dev/hud_vertical_slice_gallery.tscn"
const OUTPUT_DIRECTORY: String = "res://dev_doc/ui-art-research/hud-ghv1-vertical-slice-evidence"
...
func _initialize() -> void:
	print("=== capture_hud_vertical_slice_gallery ===")
	if DisplayServer.get_name() == "headless":
		print("HUD_VERTICAL_SLICE_CAPTURE_UNSUPPORTED_HEADLESS")
		quit(1)
		return
	if not CAPTURE_FLAG in OS.get_cmdline_user_args():
		print("HUD_VERTICAL_SLICE_CAPTURE_FLAG_REQUIRED")
		quit(1)
...
func _capture_one(packed: PackedScene, capture_size: Vector2i,
	...
	viewport.name = "CaptureViewport_%dx%d" % [capture_size.x, capture_size.y]
	...
	if not _all_boards_inside_safe_area(gallery):
		_fail("%dx%d 有板块超出安全画布" % [capture_size.x, capture_size.y])
		viewport.queue_free()
		await process_frame
		return
	...
	var filename: String = "hud_vertical_slice_%dx%d.png" % [capture_size.x, capture_size.y]
	var output_path: String = output_absolute.path_join(filename)
...
func _all_boards_inside_safe_area(gallery: Control) -> bool:
	...
	var paths: Array[String] = [
		"SafeArea/Layout/Content/SkillColumn/SkillCount5",
		"SafeArea/Layout/Content/SkillColumn/SkillCount6",
		"SafeArea/Layout/Content/SkillColumn/SkillCount7",
		"SafeArea/Layout/Content/ResourceColumn/ActionCapacity1",
		"SafeArea/Layout/Content/ResourceColumn/ActionCapacity2",
		"SafeArea/Layout/Content/ResourceColumn/ActionCapacity3",
		"SafeArea/Layout/Content/ResourceColumn/ActionMovementDisabled",
	]
	for path: String in paths:
		var board: Control = gallery.get_node_or_null(path) as Control
		if board == null or not _contains_rect(safe_rect, board.get_global_rect()):
			return false
	return true
...
func _finish() -> void:
	print("HUD_VERTICAL_SLICE_CAPTURE_OK" if not _failed else "HUD_VERTICAL_SLICE_CAPTURE_FAILED")
