...
func has_resource_region() -> bool:
	var host: Control = get_parent().get_node_or_null("ClassResourceHost")
	return host != null and host.visible


func get_frame_style_name() -> StringName:
	if has_resource_region():
		return &"panel_resources" if has_action_region() else &"collapsed_resources"
	return &"panel" if has_action_region() else &"collapsed"


func _draw() -> void:
	draw_style_box(get_frame_style(), FrameStyle.frame_rect(has_action_region(), resource_layout, has_resource_region()))


func _has_point(point: Vector2) -> bool:
	return Geometry2D.is_point_in_polygon(point, FrameStyle.outline_points(has_action_region(), resource_layout, has_resource_region()))
...
func get_logical_blocking_rects() -> Array[Rect2]:
	...
	var outline: PackedVector2Array = FrameStyle.outline_points(has_action_region(), resource_layout, has_resource_region())
	var top: int = int(FrameStyle.frame_rect(has_action_region(), resource_layout, has_resource_region()).position.y)
