...
| 印记 | `[886,520,278,56]` | 无框、无底板；三个独立布尔状态驱动原创纹章，持有时为金、青、珊瑚红，未持有时为灰色空心 |
...
当前截图由 Godot 4.6.3 的真实窗口和正式 `TacticalScene` 生成，目标显卡为 NVIDIA GeForce RTX 3070 Ti，使用 Vulkan 渲染。初始状态图来自真实场景；其余条件图使用同一场景及管理器载荷的显示副本，不代表在战斗中实际取得这些资源。
...
用户已确认第一版原创纹章与 32 像素剑气数字可以定版，正式场景默认显示。五张技能图标仍是开发预览占位；游戏正式分发前，须换成有来源、使用权与摘要记录的正式图标。集成与合并状态以 Git 和 GitHub 的实际记录为准。
