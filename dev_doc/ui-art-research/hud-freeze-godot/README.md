...
用户已确认第一版原创纹章、其约 20% 的放大尺寸与 32 像素剑气数字可以定版，正式场景默认显示。基础剑士五张技能图标已定版并进入正式战斗界面；资源包纳入与排除结果见[核验记录](../myrmidon-skill-icons/export-audit.md)，可启动的 Windows 战斗 HUD 包见[发行副本验收](../hud-release-build/README.md)。集成与合并状态以 Git 和 GitHub 的实际记录为准。
